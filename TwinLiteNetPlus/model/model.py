...
class TwinLiteNetPlus(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        # Increase capacity and add spatial context awareness
        mid_channels_fusion = 16 # Intermediate channels
        # Conv1: 4 -> mid_channels (3x3 kernel)
        self.fusion_conv1 = nn.Conv2d(chanel_img + 1, mid_channels_fusion, kernel_size=3, padding=1, bias=False)
        self.fusion_bn1 = nn.BatchNorm2d(mid_channels_fusion)
        self.fusion_act1 = nn.PReLU(mid_channels_fusion)
        # Conv2: mid_channels -> 3 (1x1 kernel)
        self.fusion_conv2 = nn.Conv2d(mid_channels_fusion, chanel_img, kernel_size=1, padding=0, bias=False)
        # --- End Modified Fusion Block ---
    ...
    def forward(self, input, heatmap):
        '''
        :param input: RGB image (B, 3, H, W)
        :param heatmap: Heatmap from object detector (B, 1, H, W), assumed normalized [0, 1]
        :return: transformed feature map
        '''
        # Concatenate along the channel dimension: (B, 3, H, W) + (B, 1, H, W) -> (B, 4, H, W)
        fused_input = torch.cat((input, heatmap), dim=1)
        # Apply the expanded fusion block
        fused_input = self.fusion_conv1(fused_input)
        fused_input = self.fusion_bn1(fused_input)
        fused_input = self.fusion_act1(fused_input)
        fused_input = self.fusion_conv2(fused_input)

        # Pass the fused input to the encoder
        out_encoder, inp1, inp2 = self.encoder(fused_input)
